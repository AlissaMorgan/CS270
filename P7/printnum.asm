...
getDigit                        ; callee setup
		...
		PUSH R5		;save FP
		ADD R5, R6, #-1 ;point to N/A local var
		
                                ; code for getDigit
		LEA R0, digits  ;load pointer to dig 0 in string
		LDR R1, R5, #4 	;load value of val into R1
		ADD R0, R0, R1	;move pointer of string based on val
		LDR R0, R0, #0	; ld in char
